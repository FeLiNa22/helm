...
| `acestream.dispatcharr.image.autoupdate.enabled`          | Whether to enable autoupdate for the Dispatcharr image.                                                                             | `false`                           |
| `acestream.dispatcharr.image.autoupdate.strategy`         | Strategy for image updates (semver, latest, newest-build, name, alphabetical, digest).                                              | `semver`                          |
| `acestream.dispatcharr.image.autoupdate.allowTags`        | Match function for allowed tags (e.g., "regexp:^[0-9]+\\.[0-9]+\\.[0-9]+$" or "any").                                              | `""`                              |
| `acestream.dispatcharr.image.autoupdate.ignoreTags`       | List of glob patterns to ignore specific tags.                                                                                      | `[]`                              |
| `acestream.dispatcharr.image.autoupdate.pullSecret`       | Pull secret name for private registries.                                                                                            | `""`                              |
| `acestream.dispatcharr.image.autoupdate.platforms`        | List of target platforms (e.g., ["linux/amd64", "linux/arm64"]).                                                                   | `[]`                              |
