...
| `sonarr.database.password`                             | Password for the external PostgreSQL database. WARNING: For production, use existingSecret instead to avoid storing passwords in values files. | `""`                         |
| `sonarr.database.existingSecret`                       | Name of an existing secret containing the database password (key: 'password'). Recommended for production.                                     | `""`                         |
...
| `prowlarr.database.password`                             | Password for the external PostgreSQL database. WARNING: For production, use existingSecret instead to avoid storing passwords in values files. | `""`                           |
| `prowlarr.database.existingSecret`                       | Name of an existing secret containing the database password (key: 'password'). Recommended for production.                                     | `""`                           |
...
| `radarr.database.password`                             | Password for the external PostgreSQL database. WARNING: For production, use existingSecret instead to avoid storing passwords in values files. | `""`                         |
| `radarr.database.existingSecret`                       | Name of an existing secret containing the database password (key: 'password'). Recommended for production.                                     | `""`                         |
...
| `lidarr.database.password`                             | Password for the external PostgreSQL database. WARNING: For production, use existingSecret instead to avoid storing passwords in values files. | `""`                         |
| `lidarr.database.existingSecret`                       | Name of an existing secret containing the database password (key: 'password'). Recommended for production.                                     | `""`                         |
...
| `acestream.image.autoupdate.enabled`                      | Whether to enable autoupdate for this service's image.                                                                              | `false`                              |
| `acestream.image.autoupdate.strategy`                     | Strategy for image updates (semver, latest, newest-build, name, alphabetical, digest).                                              | `""`                                 |
| `acestream.image.autoupdate.allowTags`                    | Match function for allowed tags (e.g., "regexp:^[0-9]+\\.[0-9]+\\.[0-9]+$" or "any").                                               | `""`                                 |
| `acestream.image.autoupdate.ignoreTags`                   | List of glob patterns to ignore specific tags.                                                                                      | `[]`                                 |
| `acestream.image.autoupdate.pullSecret`                   | Pull secret name for private registries.                                                                                            | `""`                                 |
| `acestream.image.autoupdate.platforms`                    | List of target platforms (e.g., ["linux/amd64", "linux/arm64"]).                                                                    | `[]`                                 |
...
| `acestream.securityContext.privileged`                    | Whether to run the acestream-engine container in privileged mode to allow multicast operations.                                     | `true`                               |
...
| `acestream.service.type`                                  | The type of service to create (applies to all ports).                                                                               | `LoadBalancer`                       |
...
| `acestream.service.stepdaddylivehd.port`                  | The port on which the StepDaddyLiveHD service will run.                                                                             | `3000`                               |
| `acestream.service.stepdaddylivehd.nodePort`              | The nodePort to use for the StepDaddyLiveHD service. Only used if service.type is NodePort.                                         | `""`                                 |
| `acestream.service.acexy.port`                            | The port on which the Acexy service will run.                                                                                       | `8080`                               |
| `acestream.service.acexy.nodePort`                        | The nodePort to use for the Acexy service. Only used if service.type is NodePort.                                                   | `""`                                 |
...
| `acestream.persistence.enabled`                           | Whether to enable persistence.                                                                                                      | `true`                               |
...
| `acestream.gluetun.image.autoupdate.enabled`              | Whether to enable autoupdate for the Gluetun image.                                                                                 | `false`                              |
| `acestream.gluetun.image.autoupdate.strategy`             | Strategy for image updates (semver, latest, newest-build, name, alphabetical, digest).                                              | `""`                                 |
| `acestream.gluetun.image.autoupdate.allowTags`            | Match function for allowed tags (e.g., "regexp:^[0-9]+\\.[0-9]+\\.[0-9]+$" or "any").                                               | `""`                                 |
| `acestream.gluetun.image.autoupdate.ignoreTags`           | List of glob patterns to ignore specific tags.                                                                                      | `[]`                                 |
| `acestream.gluetun.image.autoupdate.pullSecret`           | Pull secret name for private registries.                                                                                            | `""`                                 |
| `acestream.gluetun.image.autoupdate.platforms`            | List of target platforms (e.g., ["linux/amd64", "linux/arm64"]).                                                                    | `[]`                                 |
...
| `acestream.gluetun.portForwarding.enabled`                | Whether to enable VPN port forwarding for acestream (starts acestream with --port set to the forwarded port).                       | `false`                              |
...
| `acestream.stepdaddylivehd.enabled`                       | Whether to enable StepDaddyLiveHD sidecar container.                                                                                | `false`                              |
| `acestream.stepdaddylivehd.image.repository`              | The Docker repository to pull the StepDaddyLiveHD image from.                                                                       | `ghcr.io/gookie-dev/stepdaddylivehd` |
| `acestream.stepdaddylivehd.image.tag`                     | The image tag to use for StepDaddyLiveHD.                                                                                           | `latest`                             |
| `acestream.stepdaddylivehd.image.pullPolicy`              | The logic of image pulling for StepDaddyLiveHD.                                                                                     | `IfNotPresent`                       |
| `acestream.stepdaddylivehd.env.PORT`                      | The port on which StepDaddyLiveHD will listen.                                                                                      | `3000`                               |
| `acestream.stepdaddylivehd.env.API_URL`                   | The API URL for StepDaddyLiveHD (set to local IP or domain for network access).                                                     | `http://localhost:3000`              |
| `acestream.stepdaddylivehd.env.PROXY_CONTENT`             | Whether to proxy video content through the server (TRUE/FALSE).                                                                     | `TRUE`                               |
| `acestream.stepdaddylivehd.env.SOCKS5`                    | SOCKS5 proxy configuration for DLHD requests (format: user:password@proxy.example.com:1080).                                        | `""`                                 |
| `acestream.stepdaddylivehd.resources`                     | Resource limits and requests for the StepDaddyLiveHD container.                                                                     | `{}`                                 |
| `acestream.acexy.enabled`                                 | Whether to enable Acexy sidecar container (AceStream proxy for stream multiplexing).                                                | `false`                              |
| `acestream.acexy.image.repository`                        | The Docker repository to pull the Acexy image from.                                                                                 | `ghcr.io/javinator9889/acexy`        |
| `acestream.acexy.image.tag`                               | The image tag to use for Acexy.                                                                                                     | `0.2.0`                              |
| `acestream.acexy.image.pullPolicy`                        | The logic of image pulling for Acexy.                                                                                               | `IfNotPresent`                       |
| `acestream.acexy.env.ACEXY_LISTEN_ADDR`                   | Address where Acexy is listening to.                                                                                                | `:8080`                              |
| `acestream.acexy.env.ACEXY_SCHEME`                        | The scheme of the AceStream middleware (http or https).                                                                             | `http`                               |
| `acestream.acexy.env.ACEXY_HOST`                          | Where the AceStream middleware is located.                                                                                          | `localhost`                          |
| `acestream.acexy.env.ACEXY_PORT`                          | The port to connect to the AceStream middleware.                                                                                    | `6878`                               |
| `acestream.acexy.env.ACEXY_M3U8`                          | Enable M3U8 mode in Acexy (experimental).                                                                                           | `false`                              |
| `acestream.acexy.env.ACEXY_EMPTY_TIMEOUT`                 | Timeout to consider a stream is finished once empty information is received.                                                        | `60s`                                |
| `acestream.acexy.env.ACEXY_BUFFER_SIZE`                   | Buffer size for stream data.                                                                                                        | `4MB`                                |
| `acestream.acexy.env.ACEXY_NO_RESPONSE_TIMEOUT`           | Time to wait for the AceStream middleware to respond.                                                                               | `10s`                                |
| `acestream.acexy.resources`                               | Resource limits and requests for the Acexy container.                                                                               | `{}`                                 |
